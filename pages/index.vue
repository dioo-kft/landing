<template>
  <b-container fluid>
    ...
    <page-section id="intro" title="Mi a DIOO?">
      <page-intro />
    </page-section>
    <page-section id="faq" title="Gyakran Ismételt kérdések">
        <b-col class="accordion" role="tablist" align-self="stretch">
            <b-card v-for="id in items.length" :key="id" no-body class="mb-1">
                <b-card-header header-tag="header" class="p-1" role="tab">
                    <b-button block v-b-toggle="'accordion-'+id" variant="info" class="faq-question">{{ items[id-1].question }}</b-button>
                </b-card-header>
         ...
         items: [
            {
                ...
                answer: 'A DIOO érintőképernyős eszköz, egér és billentyűzet nélkül használható. Az eszköz bekapcsolása után magától elindul és készen áll a használatra.'
            },
            {
            ...
            },
            {
                ...
                answer: 'A fejlesztők a DIOO napi használati idejét 15 percben javasolják, mivel a figyelmet és koncentrálást igénylő játékfeladatok megoldása a kis felhasználókat elfárasztja. A napi játék idő, egyénként beállítható. A játékidő leteltével a gyermek új játékot aznap már nem indíthat.'
            },
            {
            ...
            },
            {
                ...
                answer: 'A DIOO használata fokozza a figyelmet, koncentrálóképességet, szókincset, a játék nyújtotta sikerélmény növeli az önbizalmat, nyitottabbá teszi a visszahúzódó gyermekeket, lassítja a túlzottan aktív gyermekeket. A DIOO-feladatok megoldása felhívhatja a figyelmet esetleges szervi problémákra (hallás, látás), képességebéli elmaradásara, vagy éppen egy-egy területen elért kiemelkedő teljesítményre. Segíti a pedagógust az egyéni fejlesztési terv meghatározásában. Fejleszti a szocializációt: a közös élmény eredményeképpen erősödik a csapatszellem: megfigyelhetően nő az egymásra való odafigyelés, az egymásnak való segítségnyújtás.'
            },
            {
